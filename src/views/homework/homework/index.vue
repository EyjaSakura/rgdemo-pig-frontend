<template>
  <div class="layout-padding">
      ...
      <el-row>
        <div class="mb8" style="width: 100%">
          ...
          <right-toolbar 
            ...
            :export="'homework_homework_export'"
        ...
        <el-table-column label="操作" width="150">
          <template #default="scope">
            <el-button 
              ...
              text 
              type="primary" 
              v-auth="'homework_homework_edit'"
              @click="formDialogRef.openDialog(scope.row.homeworkId)"
            >
              编辑
            </el-button>
            <el-button 
              ...
              type="primary" 
              v-auth="'homework_homework_del'" 
              @click="handleDelete([scope.row.homeworkId])"
            >
              删除
            </el-button>
          </template>
    ...
    <upload-excel
      ref="excelUploadRef"
      title="导入"
      url="/homework/homework/import"
      temp-url="/admin/sys-file/local/file/homework.xlsx"
      @refreshDataList="getDataList"
    />
  </div>
</template>

<script setup lang="ts" name="systemHomework">
// ========== 导入声明 ==========
import { BasicTableProps, useTable } from "/@/hooks/table";
import { fetchList, delObjs } from "/@/api/homework/homework";
 ...
 * @param objs 选中的数据行
 */
const selectionChangHandle = (objs: { homeworkId: string }[]) => {
  selectObjs.value = objs.map(({ homeworkId }) => homeworkId);
